...
#define DEVICE_NAME                     "power"                             /**< Name of device. Will be included in the advertising data. */
...
//        buffer[2] = (y_delta & 0x0ff0) >> 4;
        ...
        err_code = ble_hids_inp_rep_send(&m_hids,
                                         ...
                                         INPUT_REP_MEDIA_PLAYER_LEN,//INPUT_REP_MOVEMENT_LEN,
                                         buffer);
					}
...
static void bsp_event_handler(bsp_event_t event)
{
    ...
    switch (event)
    {
        ...
        case BSP_EVENT_KEY_0:
            if (m_conn_handle != BLE_CONN_HANDLE_INVALID)
            {
                //mouse_movement_send(-MOVEMENT_SPEED, 0);
								mouse_media_send(0x10);				// Play/Pause
            }
            break;
        ...
        case BSP_EVENT_KEY_1:
            if (m_conn_handle != BLE_CONN_HANDLE_INVALID)
            {
                //mouse_movement_send(0, -MOVEMENT_SPEED);
							mouse_media_send(0x20);				// Play/Pause
            }
            break;
        ...
        case BSP_EVENT_KEY_2:
            ...
            {
                //mouse_movement_send(MOVEMENT_SPEED, 0);
							mouse_media_send(0x40);				// Play/Pause
            }
            break;
