...
static uint8_t m_data[ANT_STANDARD_DATA_PAYLOAD_SIZE];
ant_evt_t * p_ant_evt2;
void ant_scaleable_event_handler(ant_evt_t * p_ant_evt)
{
    ...
    switch (p_ant_evt->event)
    {
        ...
        case EVENT_RX:
			p_ant_evt2 = p_ant_evt;
            message_received_handler(p_ant_evt->channel, p_ant_evt->evt_buffer);
			if( (p_ant_evt->evt_buffer[3] != m_data[0]) || (p_ant_evt->evt_buffer[4] != m_data[1]) || (p_ant_evt->evt_buffer[5] != m_data[2]) || (p_ant_evt->evt_buffer[10] != m_data[7]) ){
				for(int i = 0; i < ANT_STANDARD_DATA_PAYLOAD_SIZE;i++) {
					m_data[i] = p_ant_evt->evt_buffer[3+i];
				}
				rx_ok = 1;
			}
            break;
...
uint8_t AntRxGetFlag(void) {
	return rx_ok;
}

void AntRxSetFlag(void) {
	rx_ok = 0;
}

uint8_t AntRxGetData(uint8_t num) {
	return m_data[num];
}

void AntPush(void) {
	uint32_t err_code;
	err_code = sd_ant_broadcast_message_tx(3,
