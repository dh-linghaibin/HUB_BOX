...
#define VOLUP_BUTTON_PIN_NO             17
#define VOLDW_BUTTON_PIN_NO             18
#define PLAY_BUTTON_PIN_NO              19
...
#define DEVICE_NAME                     "Nordic_multimedia"                         /**< Name of device. Will be included in the advertising data. */
...
typedef enum
{
    ...
    CONSUMER_CTRL_VOL_UP            = 0x10,
    CONSUMER_CTRL_VOL_DW            = 0x20,
...
{
    return ble_hids_inp_rep_send(&m_hids, INPUT_REP_MPLAYER_INDEX, INPUT_REP_MEDIA_PLAYER_LEN, (uint8_t*)&cmd);
}
...
static void button_event_handler(uint8_t pin_no, uint8_t button_action)
{
    if (button_action == APP_BUTTON_PUSH)
    {
        switch (pin_no)
        {
            ...
            case DOWN_BUTTON_PIN_NO:
                mouse_movement_send(0, MOVEMENT_SPEED);
                break;
            case VOLDW_BUTTON_PIN_NO:
            {
                uint32_t err_code = consumer_control_send(CONSUMER_CTRL_VOL_DW);
                APP_ERROR_CHECK(err_code);
                break;
            }
            case VOLUP_BUTTON_PIN_NO:
            {
                uint32_t err_code = consumer_control_send(CONSUMER_CTRL_VOL_UP);
                APP_ERROR_CHECK(err_code);                
                break;
            }
            case PLAY_BUTTON_PIN_NO:
            {
                uint32_t err_code = consumer_control_send(CONSUMER_CTRL_PLAY);
                APP_ERROR_CHECK(err_code);                
                break;
            }
            default:
    ...
    if (button_action == APP_BUTTON_RELEASE)
    {
        switch (pin_no)
        {
            case VOLUP_BUTTON_PIN_NO:
            {
                uint32_t err_code = consumer_control_send(RELEASE_KEY);
                APP_ERROR_CHECK(err_code);  
                break;
            }
            case VOLDW_BUTTON_PIN_NO:
...
#warning "Pull set to match PCA10001"
        {VOLUP_BUTTON_PIN_NO,  false, GPIO_PIN_CNF_PULL_Pullup, button_event_handler},
        {VOLDW_BUTTON_PIN_NO,  false, GPIO_PIN_CNF_PULL_Pullup, button_event_handler},
        {PLAY_BUTTON_PIN_NO,  false, BUTTON_PULL, button_event_handler},
    };
